...
import com.ccerp.bean.services.SupmedServicesBeanBase;

@SuppressWarnings("deprecation")
public class SupmedView extends AbstractExcelView {
	@Override
	protected void buildExcelDocument(Map<String, Object> map,
			HSSFWorkbook workBook, HttpServletRequest req,
			HttpServletResponse rep) throws Exception {
		@SuppressWarnings("unchecked")
		List<SupmedServicesBeanBase> list = (List<SupmedServicesBeanBase>) map
				.get("supmedList");
		...
		String[] names = new String[] { "序号", "代垫单位", "生效日期", "支付单位",
				"员工所在部门", "员工姓名", "员工编号", "补充医疗保险个人", "补充医疗保险单位" };
		for (int i = 0; i < 9; i++) {
		...
		if (list != null && list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				SupmedServicesBeanBase psb = list.get(i);
				for (int j = 0; j < 9; j++) {
					...
					if (j == 7) {
						cell.setCellValue(psb.getSupmedicalEe());
					}
					if (j == 8) {
						cell.setCellValue(psb.getSupmedicalEr());
					}
				}
			}
		}

		String filename = "补充医疗数据导出-"
